...
app.post('/login', (req, response) => {
    if (req.body.user == "NEWDAY" && req.body.pass == "1234") {
        console.log(`login pass`);
        const token = jwt.sign({ username: "NEWDAY" }, jwtsecret)
        console.log(token);
...
app.put('/put', (req, response) => {
    ...
    sqlpool.query(sql, {
        ...
        return response.json({ data: "succeed" })
    })
})

app.delete('/delete', (req, response) => {
    if(!req.body.id) return response.status(400).send("Error data");
